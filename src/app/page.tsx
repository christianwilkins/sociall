...
const copyToClipboard = (text: string, index: number) => {
  navigator.clipboard.writeText(text)
    .then(() => {
      ...
      setCopiedIndex(index); // Set the index of the copied card
    ...
    if (response.ok) {
      ...
      const normalizeKey = (key: string) => {
        const specialCases: { [key: string]: string } = {
          linkedin: 'LinkedIn',
          // You can add more special cases here if needed
        };
        return specialCases[key.toLowerCase()] || key.charAt(0).toUpperCase() + key.slice(1);
      };
      
      const parseData = (jsonData: { [key: string]: any }): { [key: string]: any } => {
        return Object.keys(jsonData).reduce<{ [key: string]: any }>((acc, key) => {
          const normalizedKey = normalizeKey(key);
          acc[normalizedKey] = jsonData[key];
          return acc;
        }, {});
      };
      
      const lowercaseData = JSON.parse(data.description);
      const parsedData = parseData(lowercaseData);
        ...
        {file && showAnimation && (
          ...
          <div className="flex flex-wrap gap-4 justify-center confirmation-message slide-up-animation">
            ...
            {cardsData.map((card, index) => (
              <div
                key={index}
                className="relative w-[16rem] md:w-[20rem] h-[18rem] m-2 border border-gray-200 rounded-lg overflow-hidden shadow-lg transition-shadow duration-300 ease-in-out hover:shadow-xl hover:scale-105 transform-gpu"
                  ...
                  <div className="flex flex-col h-full">
                    <Card className="h-full">
                      <CardHeader>
                        ...
                        <CardDescription className="overflow-auto">{card.description}</CardDescription>
                      </CardHeader>
                      {/* Optionally include CardContent and CardFooter here */}
                    </Card>
                    <Button 
                      onClick={() => copyToClipboard(card.description, index)}
                      className={`absolute top-2 right-2 p-2 text-xs rounded-md z-10 ${
